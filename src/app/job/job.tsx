...
const Job = () => {
  ...
  return (
    <div className="pt-10 flex items-center justify-center flex-wrap">
      <div className="bg-[#EAF0FF] bg-opacity-5 xl:w-[18
      00px] max-w-[1000px] flex flex-wrap flex-col items-center justify-center p-10 rounded-md gap-10">
        ...
        <div className="flex flex-wrap gap-5 items-center justify-center">
          {countries.map((item, index) => (
            <div key={index} className="w-[270px] pl-8 pr-8 pt-4 pb-4 flex flex-wrap bg-[#EAF0FF] bg-opacity-10 flex-col gap-8 rounded-md">
              <Image src={item.flag} alt={`${item.name} flag`} className="w-8 h-8 rounded-md" />
              <h2 className="font-medium text-[20px] opacity-90">{item.name}</h2>
